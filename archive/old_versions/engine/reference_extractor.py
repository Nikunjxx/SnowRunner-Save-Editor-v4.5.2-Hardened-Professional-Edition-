...
class ReferenceExtractor:
    ...
    def extract_all(self, force: bool = False):
            ...
            try:
                with open(self.cache_file, 'r') as f:
                    ...
                    if cached_data.get("version") == self.current_version:
                        print(f"[Extractor] Cache version matches {self.current_version}. Skipping.")
                        return
            except Exception:
                pass

        print(f"[Extractor] Starting deep scan of {self.remote2_path}")
        if not os.path.isdir(self.remote2_path):
            print(f"[Extractor] Warning: {self.remote2_path} not found. Using partial defaults.")
            self._save_cache() # Save empty shell with version
            return

        for root, dirs, files in os.walk(self.remote2_path):
            # Ignore backup directories within Remote2 (Edge-case hardening)
            if 'backup' in root.lower() or 'temp' in root.lower():
                continue
                
            for file in files:
                if not (file.endswith('.cfg') or file.endswith('.dat')):
                    continue
        ...
        self._save_cache()
        print(f"[Extractor] Extraction complete. Pattern cache saved to {self.cache_file}")
