...
# 1. Precision Fog Patching (v110.5)
# Replacing the entire generate_revealed_fog function with the header-preserved selective patching logic
new_fog_func = r'''def generate_revealed_fog(save_path, selected_regions, notify=True):
    """
    ...
    while preserving every other byte (e.g. 0x80 or header metadata).
    This ensures 100% compatibility with maps like Smithville Dam (589x589).
    """
...
# Using string markers to replace the function block safely
...
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + new_fog_func + content[end_idx + len(end_marker):]
    print("Injected v110.5 fog logic.")
...
with open(target, "w", encoding="utf-8") as f:
    f.write(content)
print("v110.5 deployment successfully patched.")
