...
class Validator:
    ...
    def validate_vehicle_mutation(self, region: str, mutation_data: Dict[str, Any]) -> ValidationReport:
        """
        [v110.32] Tiered Vehicle Validation:
        1. Coordinate Bounding Box (2048x2048)
        2. Fuel/Damage Logical Bounds
        """
        report = ValidationReport()
        
        # 1. Coordinate Validation
        if "x" in mutation_data and "z" in mutation_data:
            x, z = float(mutation_data["x"]), float(mutation_data["z"])
            # Typical SnowRunner maps are 1km or 2km square. 
            # (0,0) is center, so bounds are -1024 to +1024.
            LIMIT = 1024.0 
            if abs(x) > LIMIT or abs(z) > LIMIT:
                report.add_warning(f"Coordinate Out-of-Bounds: ({x}, {z}) exceeds nominal map limits (Â±{LIMIT}). Vehicle may spawn in 'The Void'.")
            
            # Altitude Check (y)
            if "y" in mutation_data:
                y = float(mutation_data["y"])
                if y < -50 or y > 500:
                    report.add_warning(f"Altitude Warning: {y}m is unusually high/low. Vehicle may fall or be stuck in terrain.")

        # 2. Damage/Fuel Validation
        if "damage" in mutation_data:
            d = mutation_data["damage"]
            if d < 0: report.add_error("Invalid Mutation: Damage cannot be negative.")
